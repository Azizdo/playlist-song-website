...
class SongService {
  ...
  async getSongById (id) {
    const songs = await this.collection.findOne({ id: parseInt(id) });
    return songs;
  }
  ...
  async updateSongLike (id) {
    const song = await this.collection.findOne({ id: parseInt(id) });
    const likedState = !song.liked;
    await this.collection.updateOne({ id: parseInt(id) }, { $set: { liked: likedState } });
    return likedState;
  }
  ...
  async search (substring, exact) {
    ...
    if (!exact) {
      filter = {
        $or: [{ name: { $regex: `${substring}`, $options: "i" } }, { genre: { $regex: `${substring}`, $options: "i" } }, { artist: { $regex: `${substring}`, $options: "i" } }]
      };
    } else {
      filter = {
        $or: [{ name: { $regex: `${substring}` } }, { genre: { $regex: `${substring}` } }, { artist: { $regex: `${substring}` } }]
      };
    }
