...
describe.only("Player tests", () => {
  ...
  it("getSongFromIndex should get song for a valid index", () => {
    ...
    for (let i = 0; i < songStubs.length; i++) {
      expect(player.getSongFromIndex(i)).toEqual(songStubs[i]);
    }
  });
  ...
  it("playPreviousSong should call playAudio", () => {
    player.loadSongs(songStubs);
    const playerAudioPlaySpy = jest.spyOn(player, "playAudio");
  ...
  it("playPreviousSong should decrement currentIndex when shuffle is disabled", () => {
    ...
    const previousIndex = player.currentIndex;
    player.playPreviousSong();
    expect(player.currentIndex).toEqual((previousIndex - 1 + songStubs.length) % (songStubs.length));
  });
  ...
  it("scrubTime should correctly add delta stepper", () => {
    // TODO
    const delta = 5;
    const previousTime = player.audio.currentTime;
    player.scrubTime(delta);
    expect(player.audio.currentTime).toEqual(previousTime + delta);
  });
