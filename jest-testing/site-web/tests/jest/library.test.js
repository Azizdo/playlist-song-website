...
describe("Library tests", () => {
  ...
  const setUpHTML = () => {
    ...
    songContainer.setAttribute("class", "flex-row");
  ...
  it("generateLists should call buildPlaylistItem and buildSongItem, and append children to containers", () => {
    ...
    expect(playlistCont.hasChildNodes()).toBe(true);
    expect(songCont.hasChildNodes()).toBe(true);
  });
  ...
  it("buildSongItem should build song's item", () => {
    ...
    expect(songItem.innerHTML).toEqual(
      `<p>${song.name}</p><p>${song.genre}</p><p>${song.artist}</p><button class="fa-heart fa-2x ${song.liked ? "fa" : "fa-regular"}"></button>`
    );
  });

  it("buildSongItem should add a call to StorageManager.replaceItem on click event and change the classList", () => {
    // TODO
    // clicker sur la chanson ne trigger rien, on assume que le click event et sur le likedbutton;

    const replaceItemSpy = jest.spyOn(library.storageManager, "replaceItem").mockImplementation(() => {});
    const song = library.buildSongItem(songs[0]);
    document.getElementById("song-container").appendChild(song);
    const button = document.getElementById("song-container").querySelector("button");

    expect(button.classList.contains("fa-regular")).toBe(true);
    expect(button.classList.contains("fa")).toBe(false);

    button.click();

    expect(replaceItemSpy).toBeCalled();
    expect(button.classList.contains("fa-regular")).toBe(false);
