...
export class Library {
  ...
  searchInFields (searchFields, searchValue, exactMatch) {
    for (const searchField of searchFields) {
      if (this.includesSubstring(searchField, searchValue, exactMatch)) {
        return true;
      }
    }
    return false;
  ...
  search (searchInput, searchSources, exactMatch) {
    ...
    for (const playlist of searchSources.playlists) {
      if (this.searchInFields([playlist.name, playlist.description], searchInput.value, exactMatch)) {
        playlists.push(playlist);
      }
    }

    for (const song of searchSources.songs) {
      if (this.searchInFields([song.name, song.artist, song.genre], searchInput.value, exactMatch)) {
        songs.push(song);
      }
    }
